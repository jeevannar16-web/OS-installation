...
import windowsConfig from "../src/data/windows";
...
export default function Windows11VM({ onDone }: { onDone?: () => void }) {
  const cfg = windowsConfig;
  const [step, setStep] = useState<Step>("name");
  const [vmName, setVmName] = useState(cfg.branding.shortName + " VM");
  const [memory, setMemory] = useState(cfg.vmConfig.defaultMemoryMB);
  const [cpus, setCpus] = useState(2);
  const [efi, setEfi] = useState(true);
  const [diskGB, setDiskGB] = useState(cfg.vmConfig.defaultDiskGB);
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          <select className="flex-1 border border-gray-300 rounded px-3 py-2 text-[11px] text-gray-800 outline-none cursor-pointer bg-white">
                            ...
                            <option>{cfg.iso.filename} ({cfg.iso.size})</option>
                          </select>
                          <button className="px-3 py-2 border border-gray-300 rounded bg-white hover:bg-gray-100 text-xs">📂</button>
                        </div>
                      </div>
                  ...
                  {step === "summary" && (
                    <div className="space-y-3">
                      <div className="bg-gray-50 border border-gray-200 rounded-lg divide-y divide-gray-100">
                        {[["Name", vmName],["Machine Folder", folder],["ISO Image", cfg.iso.filename],["Type / Version", `Microsoft Windows - Windows 11 (64-bit)`],["Memory", memory >= 1024 ? `${(memory / 1024).toFixed(1)} GB` : `${memory} MB`],["Processors", `${cpus} CPU${cpus > 1 ? "s" : ""}`],["EFI", efi ? "Enabled" : "Disabled"],["Disk", createDisk ? `${diskGB} GB ${diskType} (${diskAlloc === "dyn" ? "Dynamic" : "Fixed"})` : "Existing file"]].map(([l, v]) => (
