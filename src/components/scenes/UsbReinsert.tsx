...
export default function UsbReinsert({ onComplete }: { onComplete: () => void }) {
  ...
  const demoSteps: Array<{
    ...
    { type: "move", x: 250, y: 350, delay: 0, duration: 500 },
    { type: "drag", x: 250, y: 350, delay: 300, duration: 600, dragEndX: 550, dragEndY: 350 },
  ];
          ...
          <div className="text-sm lg:text-base text-white/50">The system is ready to reboot into the installer.</div>
            <PulseHint>
              <button
                ...
                className="mt-4 rounded-xl bg-accent px-8 py-3 text-sm font-bold text-white hover:bg-accent-soft transition-colors shadow-lg shadow-accent/20"
              >
                Reboot now →
              </button>
            </PulseHint>
