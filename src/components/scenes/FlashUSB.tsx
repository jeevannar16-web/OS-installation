  ...
  useEffect(() => {
    ...
    const logInterval = setInterval(() => {
      setLogLines((prev) => {
        ...
        if (idx < logSequence.length) return [...prev, logSequence[idx]];
        return prev;
      });
  ...
  return (
    <div className="w-full">
      <div className="rounded-xl bg-[#f0f0f0] ring-1 ring-black/10 overflow-hidden">
        {/* Title bar */}
        <div className="flex items-center gap-2 bg-[#e0e0e0] px-3 py-1.5 text-xs text-gray-700">
          <span className="font-bold">Rufus</span>
          <span className="text-gray-500">v4.5.2180</span>
        </div>

        <div className="bg-[#f5f5f5] p-4 space-y-3">
          {/* Device dropdown */}
          <div className="flex items-center gap-3">
            <label className="w-24 text-xs font-semibold text-gray-600">Device</label>
            <select
              value={device}
          ...
          {/* Boot selection */}
          <div className="flex items-center gap-3">
            <label className="w-24 text-xs font-semibold text-gray-600">Boot selection</label>
            <button
            ...
            <button
              onClick={() => setPickerOpen(true)}
              className="rounded bg-[#6c5ce7] px-4 py-1.5 text-xs font-bold text-white hover:bg-[#5a4bd1]"
            >
              SELECT
            </button>
          </div>
          ...
          {/* Partition scheme */}
          <div className="flex items-center gap-3">
            <label className="w-24 text-xs font-semibold text-gray-600">Partition scheme</label>
            <select
              value={partScheme}
          ...
          {/* File system */}
          <div className="flex items-center gap-3">
            <label className="w-24 text-xs font-semibold text-gray-600">File system</label>
            <select
              value={fileSys}
          ...
          {/* Progress bar (when flashing) */}
          {(rufusPhase === "flashing" || rufusPhase === "done") && (
            <div className="space-y-1">
              <div className="h-2 w-full overflow-hidden rounded bg-gray-200">
                <motion.div
                  className="h-full rounded bg-[#6c5ce7]"
          ...
          {rufusPhase !== "flashing" && rufusPhase !== "done" && (
            <button
              ...
              onClick={() => {
                if (isoFile && device) setRufusPhase("flashing");
              }}
              className="w-full rounded py-2 text-sm font-bold text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed bg-[#6c5ce7] hover:bg-[#5a4bd1]"
            >
              START
            </button>
          )}
...
function VentoyTool({
  ...
  return (
    <div className="rounded-xl bg-[#1a1a2e] ring-1 ring-white/10 overflow-hidden p-5 space-y-4">
      ...
      {ventoyPhase === "idle" && (
        <button
          onClick={() => setVentoyPhase("installing")}
          ...
          <div className="flex gap-6 items-start">
            ...
            <div
              ...
              onDrop={(e) => {
                ...
                setTimeout(() => setVentoyPhase("done"), copyDuration);
              }}
...
function EtcherTool({
  ...
  const steps = [
    ...
    },
    {
      ...
      onClick: () => {
        if (selectedFile && selectedTarget) setEtcherPhase("flashing");
      },
      icon: "⚡",
  ...
  return (
    <div className="rounded-xl bg-[#1a1a2e] ring-1 ring-white/10 overflow-hidden p-5 space-y-4">
        ...
        >
          ...
          {USB_DEVICES.map((d) => (
            <button
              key={d.id}
              onClick={() => {
                setSelectedTarget(d.id);
                setEtcherPhase("pick_file");
              }}
      ...
      {etcherPhase === "done" && (
        ...
        >
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500 text-xl text-white">
            ✓
          </div>
          <div>
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="relative flex items-center gap-24 py-8">
              ...
              <motion.div
                ...
                animate={dragging ? { scale: 1.05 } : { scale: 1, y: [0, -4, 0] }}
                transition={
                  dragging
                    ? {}
        ...
        {(phase === "rufus" || phase === "ventoy" || phase === "balena") && (
          ...
          >
            ...
            {phase === "rufus" && (
              <RufusTool config={config} speed={speed} />
            )}
            {phase === "ventoy" && (
              <VentoyTool config={config} speed={speed} />
            )}
            {phase === "balena" && (
              <EtcherTool config={config} speed={speed} />
            )}

            {/* Safely Eject — appears after tool is done */}
