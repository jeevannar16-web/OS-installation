...
const ARCH_STEPS: GuideStep[] = [
  {
    ...
    tip: "Verify the checksum after downloading. Use 'certUtil -hashfile archlinux-*.iso SHA256' on Windows or 'sha256sum archlinux-*.iso' on Linux.",
  },
  {
    title: "Boot from USB",
    description: "Use Rufus (Windows), Ventoy, or dd to write the ISO to a USB drive. Restart, enter BIOS (F2/Del/F12), set USB as first boot device.",
    kind: "info",
    guiAction: "Rufus → Device: USB → SELECT → choose ISO → DD Image mode → START → OK",
  ...
  },
  {
    ...
    description: "The USB boots into the Arch live environment. Kernel messages scroll by, then you land at a root shell prompt. No password needed.",
    kind: "info",
    image: "/images/arch/02-live-env.png",
  },
  {
    title: "Connect to Network",
    description: "Use iwctl for WiFi or nmtui for a more friendly interface. For Ethernet, it's usually automatic.",
    kind: "command",
    commands: [
      "# Option A: nmtui (recommended)",
      "nmtui",
      "# Option B: iwctl (manual)",
      "iwctl",
      "[iwd]# device list",
      "[iwd]# station wlan0 scan",
      "[iwd]# station wlan0 get-networks",
      "[iwd]# station wlan0 connect \"YourWiFi\"",
      "[iwd]# exit",
    ],
    output: "Connected successfully",
    tip: "Use 'ip link' to find your interface name (wlan0, wlp2s0). Run 'ping archlinux.org' to verify connectivity.",
  },
  {
    title: "Launch archinstall",
    description: "Run the guided installer. It opens a TUI menu where you configure everything before installing.",
    kind: "gui",
    guiAction: "Type: archinstall → Press Enter → (TUI menu opens)",
  ...
  },
  {
    ...
    description: "Set keyboard layout and locale language.",
    kind: "gui",
    guiAction: "Locales → Keyboard layout: us → Locale language: en_US.UTF-8 → Back",
  ...
  },
  {
    ...
    kind: "gui",
    guiAction: "Mirrors → Select your region (e.g. United States, India) → Back",
  ...
  },
  {
    ...
    kind: "gui",
    guiAction: "Disk configuration → Best effort → Select drive (e.g. /dev/sda) → Filesystem: ext4 → Confirm",
  ...
  },
  {
    ...
    description: "Confirm filesystem and set up swap (zram recommended for modern systems).",
    kind: "gui",
    guiAction: "Disk config → Filesystem: ext4 → Swap: zram → Confirm",
  ...
  },
  {
    ...
    tip: "Use GRUB for dual-boot. Use systemd-boot for UEFI-only single-boot setups.",
  },
  {
    title: "User Account",
    description: "Set the hostname, root password, and create your user account with sudo access.",
    kind: "gui",
    guiAction: "Hostname → archlinux → Root password → [set] → User account → Add user → [username] → [password] → Super user: Yes → Confirm",
    image: "/images/arch/12-user-accounts.png",
  },
  {
    title: "Set Passwords",
    description: "Enter and confirm strong passwords for root and your user account.",
    kind: "gui",
    guiAction: "Root password → [strong password] → User password → [strong password] → Confirm",
    image: "/images/arch/11-user-password.png",
  },
  {
    title: "Desktop Environment",
    description: "Choose a desktop profile — KDE Plasma, GNOME, XFCE, or i3.",
    kind: "gui",
    guiAction: "Profile → Type: Desktop → KDE Plasma → Graphics: All open source → Audio: PipeWire → Back",
    image: "/images/arch/12-desktop-profile.png",
    tip: "KDE is most feature-rich. GNOME is simpler. XFCE is lightweight for older hardware.",
  },
  {
    title: "Optional Repositories",
    description: "Enable multilib for 32-bit packages (needed for Steam, Wine, gaming).",
    kind: "gui",
  ...
  },
  {
    ...
    description: "Choose which kernel(s) to install. linux (default) is recommended.",
    kind: "gui",
    guiAction: "Kernels → linux → optional: linux-lts or linux-zen → Confirm",
    image: "/images/arch/10-select-kernels.png",
  },
  {
    title: "Additional Packages",
    description: "Add extra packages. NetworkManager is required for WiFi. Add firefox for a web browser.",
    kind: "gui",
  ...
  },
  {
    ...
    kind: "gui",
    guiAction: "Timezone → Select your timezone (e.g. Asia/Kolkata) → NTP: True → Install → Confirm",
    image: "/images/arch/14-confirm-install.png",
    tip: "This is the point of no return. The installer will format partitions and install the system.",
  },
  {
    title: "Installation Progress",
    description: "archinstall partitions the disks, installs packages, configures bootloader, and sets up your desktop. Takes 5-15 minutes.",
    kind: "info",
    image: "/images/arch/15-install-progress.png",
    tip: "Do NOT power off during installation. Wait for 'Press any key to reboot'.",
  },
  {
    title: "Reboot into Arch",
    description: "After installation completes, exit and reboot. Remove the USB drive when prompted.",
    kind: "command",
    commands: [
      "# After 'Press any key to reboot'",
      "umount -R /mnt",
      "reboot",
    ],
    image: "/images/arch/13-reboot.png",
    tip: "If it boots back into USB, power off and remove the USB, then restart.",
  },
  {
    title: "GRUB & First Boot",
    description: "GRUB shows Arch Linux in the boot menu. Select it to boot into your new system.",
    kind: "info",
    image: "/images/arch/17-reboot-grub.png",
    tip: "If dual-booting, Windows will also appear in the GRUB menu. Login with the user you created.",
  },
  {
    title: "Login & Desktop",
    description: "The display manager (SDDM for KDE, GDM for GNOME) shows a login screen. Enter your password.",
    kind: "info",
    image: "/images/arch/14-login.png",
    tip: "After login, run 'sudo pacman -Syu' to update everything. Welcome to Arch Linux!",
  },
];
