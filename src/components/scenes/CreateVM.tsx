  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="flex-1 flex flex-col rounded-2xl border border-gray-600/30 shadow-2xl font-sans overflow-hidden select-none"
        ...
        {/* Title bar — exact VirtualBox 7.0 style */}
          ...
          <div className="flex-1 bg-[#f0f0f0] p-6 overflow-y-auto">
            <div className="max-w-2xl mx-auto">
              ...
              <p className="text-xs text-gray-500 mb-5">
                {step === "name" && "Enter a name for the virtual machine and choose the operating system you plan to install."}
                {step === "memory" && "Select the amount of RAM (megabytes) to allocate to the virtual machine."}
                {step === "disk" && "Select a virtual hard disk for the machine. You can create a new disk or use an existing one."}
                {step === "summary" && "Review the configuration below before creating the virtual machine."}
              </p>
                ...
                <motion.div key={step} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.12 }}>
                  {step === "name" && (
                    <div className="space-y-4">
                      ...
                      <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-[11px] text-blue-800/70 leading-relaxed">
                  ...
                  {step === "memory" && (
                    <div className="space-y-4">
                      <div className="flex items-center gap-5">
                        ...
                        <div className="bg-white border border-gray-300 rounded px-4 py-2 text-sm font-semibold text-gray-800 min-w-[90px] text-center">
                      ...
                      <p className="text-[11px] text-gray-400">
                        ...
                        &nbsp;|&nbsp; Min: 512 MB | Max: 16 GB
                      </p>
                      <div className="bg-amber-50 border border-amber-100 rounded-lg p-3 text-[11px] text-amber-800/70 leading-relaxed">
                    ...
                    <div className="space-y-4">
                      ...
                      {createDisk && (
                        <div className="space-y-4 pl-1">
                          <div>
                            <label className="block text-xs font-semibold text-gray-600 mb-1">Disk size:</label>
                            <div className="flex items-center gap-5">
                              <input type="range" min={4} max={256} step={1} value={diskGB}
                                onChange={e => setDiskGB(Number(e.target.value))}
                                className="flex-1 accent-[#4a8cff] h-2" />
                              <div className="bg-white border border-gray-300 rounded px-4 py-2 text-sm font-semibold text-gray-800 min-w-[70px] text-center">
                                {diskGB} GB
                              </div>
                            </div>
                      ...
                      <div className="bg-white border border-gray-200 rounded-lg divide-y divide-gray-100">
                        {[
                          ...
                          <div key={l} className="flex justify-between px-4 py-2.5 text-sm">
                            ...
                            <span className="text-gray-800 font-semibold max-w-[60%] text-right truncate">{v}</span>
                          </div>
                        ))}
                      </div>
        ...
        <div className="bg-[#2a2a2a] border-t border-gray-600/40 px-3 py-1 flex items-center gap-2 shrink-0">
          ...
          <div className="flex-1" />
          <span className="text-[8px] text-gray-500">Step {currentIdx + 1} / {steps.length}</span>
        </div>
      </div>
    </div>
  );
