...
export default function Partition({
  ...
  if (!diskShrunk) {
    return (
      <div className="mx-auto w-full max-w-xl border border-red-500/20 bg-red-500/50/5 p-6 rounded-2xl text-center space-y-4 shadow-xl">
  ...
  return (
    <div className="mx-auto w-full max-w-4xl lg:max-w-5xl space-y-5">
      <div>
        <h2 className="text-xl font-bold text-white/90">Manual partitioning</h2>
      ...
      <div className="space-y-2">
        <div className="text-xs font-medium text-white/50 mb-1">/dev/sda — {TOTAL_GB} GB</div>
        <div className="flex h-10 lg:h-12 w-full overflow-hidden rounded-lg border border-white/10 bg-white/[0.06]">
          {partitions.map((p, i) => {
            ...
            const bg = colors[p.fs as keyof typeof colors] || "bg-white/10";
            return (
              <div
                key={i}
                className={`${bg} flex items-center justify-center text-[10px] font-medium text-white/80 border-r border-white/50 overflow-hidden whitespace-nowrap`}
                style={{ width: `${pct}%` }}
          ...
          {freeGB > 0 && (
            <div
              className="flex items-center justify-center text-[10px] font-medium text-white/40 border-r border-white/50 border-dashed"
      ...
      <div className="rounded-xl border border-white/10 overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-white/[0.03] border-b border-white/10 text-left text-xs font-medium text-white/50 uppercase tracking-wider">
          ...
          <tbody className="divide-y divide-white/5">
            {partitions.map((p, i) => (
              <tr key={i} className={`${p.mount === "/" ? "bg-emerald-500/5" : ""} hover:bg-white/[0.05] transition-colors`}>
                <td className="px-4 py-2.5 font-mono text-xs text-white/60">{p.device}</td>
                <td className="px-4 py-2.5 text-white/80">{p.type}</td>
                <td className="px-4 py-2.5">
                  {p.fs ? (
                    <span className="inline-block rounded bg-white/[0.06] px-2 py-0.5 text-xs font-medium text-white/60">{p.fs}</span>
                  ) : (
                    <span className="text-white/30">—</span>
                  )}
                </td>
                <td className="px-4 py-2.5 text-white/60">{p.sizeGB} GB</td>
                <td className="px-4 py-2.5">
                  {p.mount ? (
                    <span className={`font-mono text-xs ${p.mount === "/" ? "text-emerald-400 font-semibold" : "text-white/60"}`}>
                      {p.mount}
                    </span>
                  ) : (
                    <span className="text-white/30">—</span>
                ...
                <td className="px-4 py-2.5 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button
                      ...
                      className="rounded px-2 py-1 text-xs text-white/50 hover:bg-white/[0.06] hover:text-white/80 transition-colors"
                    >
                      Edit
                    </button>
                    {deleteConfirm === i ? (
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="rounded px-2 py-1 text-xs text-red-400 bg-red-500/5 hover:bg-red-500/10 transition-colors font-medium"
                        >
                          Delete
                        </button>
                        <button
                          onClick={() => setDeleteConfirm(null)}
                          className="rounded px-2 py-1 text-xs text-white/50 hover:bg-white/[0.06] transition-colors"
                        >
                          Cancel
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="rounded px-2 py-1 text-xs text-white/40 hover:bg-red-500/5 hover:text-red-500 transition-colors"
                      >
                        Delete
                      </button>
                    )}
            ...
            {freeGB > 0 && (
              <tr className="bg-white/[0.03]">
                <td className="px-4 py-2.5 font-mono text-xs text-white/40">—</td>
                <td className="px-4 py-2.5 text-white/40 italic text-xs">Free Space</td>
                <td className="px-4 py-2.5 text-white/30">—</td>
                <td className="px-4 py-2.5 text-white/40">{freeGB.toFixed(1)} GB</td>
                <td className="px-4 py-2.5 text-white/30">—</td>
                <td className="px-4 py-2.5" />
              </tr>
            )}
          ...
          >
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-white/60 mb-1">Size (GB)</label>
                  <input
                    type="number"
                    min={1}
                    ...
                    className="w-full rounded-lg border border-white/15 px-3 py-2 text-sm text-white/90 outline-none focus:border-[#E95420] focus:ring-1 focus:ring-[#E95420]/30"
                  />
                ...
                <div>
                  <label className="block text-xs font-medium text-white/60 mb-1">Filesystem</label>
                  <select
                    value={form.fs}
                    onChange={(e) => setForm((p) => ({ ...p, fs: e.target.value }))}
                    className="w-full rounded-lg border border-white/15 px-3 py-2 text-sm text-white/90 outline-none focus:border-[#E95420] focus:ring-1 focus:ring-[#E95420]/30 bg-[#12121a]"
                  >
                    {FILESYSTEMS.map((fs) => (
                ...
                <div>
                  <label className="block text-xs font-medium text-white/60 mb-1">Mount point</label>
                  <select
                    value={form.mount}
              ...
              <div className="flex justify-end gap-2 mt-6">
                <button
                  ...
                  className="rounded-lg border border-white/15 px-4 py-2 text-sm font-medium text-white/60 hover:bg-white/[0.03] transition-colors"
                >
                  Cancel
                </button>
                <button
      ...
      <div className="flex flex-col items-center gap-2 pt-2">
        {!canConfirm && (
          <div className="text-xs text-amber-400 bg-amber-500/5 border border-amber-500/20 rounded-lg px-3 py-1.5">
